from sqlalchemy.ext.asyncio import async_engine_from_config
from telegram import InlineKeyboardButton, InlineKeyboardMarkup
from telegram.ext import ContextTypes

from Admin.database.actionWithDB.actionWithChannelDB import append_in_db_channel, del_from_db_channel
from Admin.database.actionWithDB.config_for_DB import get_from_db_data

BUTTON, BACK = range(2)

async def get_topics():

    channel = "topics"

    channels = await get_from_db_data(channel)
    tempalte = ""

    count = 1
    for i, j in channels.items():
        tempalte += f"{count}) {j}\n"
        count += 1

    return tempalte

# Просмотр
async def handler_view_topics(update, context):

    query = update.callback_query
    await query.answer()

    keyboard = [[InlineKeyboardButton("<< В меню", callback_data='BACK')]]
    reply_markup = InlineKeyboardMarkup(keyboard)

    text = await get_topics()

    await query.edit_message_text(f"Текущие топики/темы:\n\n{text}", reply_markup=reply_markup)


async def select_channel(update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()

    from_user = query.from_user.id
    get_sent_channel = query.message.reply_markup.inline_keyboard[0][0].text
    context.user_data["sent_channel"] = get_sent_channel
    print(context.user_data)

    await context.bot.send_message(from_user, "Напиши название нового топика")



async def handler_create_topic(update, context):

    user_input = update.message.text
    selected_channel = "@" + context.user_data['sent_channel']

    print(selected_channel)
    await context.bot.createForumTopic(chat_id=selected_channel, name=user_input)
    # get_sender_channel = query.message.reply_markup.inline_keyboard[0][0].text

    context.user_data.clear()
    print(context.user_data)


# Добавление
async def handler_add_topic(update, context):

    keyboard = [[InlineKeyboardButton("<< В меню", callback_data='BACK')]]
    reply_markup = InlineKeyboardMarkup(keyboard)

    channel = {}

    user_input = update.message.text

    try:
        data_channel = await context.bot.get_chat(chat_id=f"@{user_input}")

    except:
        await update.message.reply_text("Канал не найден. Некоректное имя",
                                        reply_markup=reply_markup)
        return BACK


    channel[user_input] = data_channel.id

    try:
        await append_in_db_channel(channel)

        text = await get_topics()

        await update.message.reply_text(f"Канал `{user_input}` добавлен. Текущие каналы:\n\n{text}", reply_markup=reply_markup)

    except:
        await update.message.reply_text("Возникла проблема при добавлении названия в БД. Обратитесть к админу Бота", reply_markup=reply_markup)

    return BACK

# Удаление
async def handler_delete_topic(update, context):

    user_input = update.message.text

    keyboard = [[InlineKeyboardButton("<< В меню", callback_data='BACK')]]
    reply_markup = InlineKeyboardMarkup(keyboard)

    try:
        await del_from_db_channel(user_input)

        text = await get_topics()

        await update.message.reply_text(f"Канал `{user_input}` удалён. Текущие каналы:\n\n{text}",
                                        reply_markup=reply_markup)
    except:
        await update.message.reply_text("Такой канал отсутствует", reply_markup=reply_markup)

    return BACK
